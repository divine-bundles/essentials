...
If _Divine.dotfiles_ is already installed, the following Terminal command *attaches* the bundle `essentials` to the link:{url_dd}#fmwk-grail[Grail directory]:
...
And this Terminal command *detaches the bundle* from the link:{url_dd}#fmwk-grail[Grail directory]:
